          ...
          <nav className="flex gap-1 border-b border-border">
            {tabs.map((tab) => (
              <button
                ...
                className={`px-4 py-2 text-sm font-[family-name:var(--font-mono)] transition-colors ${
                  activeTab === tab.id
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-background rounded-lg p-4 border border-border">
                  <h4 className="text-sm font-semibold text-accent font-[family-name:var(--font-mono)] mb-2">
                    Problem
                  </h4>
                    ...
                    {screenshots.length > 1 && (
                      <div className="absolute bottom-0 left-0 right-0 flex items-center justify-between p-2 bg-background/80 backdrop-blur-sm">
                        <button
                          ...
                          className="px-3 py-1 text-xs font-[family-name:var(--font-mono)] text-muted hover:text-accent bg-surface rounded border border-border transition-colors"
                        >
                          ← Prev
                        </button>
                ...
                <div className="bg-background rounded-lg border border-border p-6">
                  <div className="flex flex-wrap gap-4 justify-center">
                    {architecture.map((node) => (
                      <div
                        key={node.id}
                        className="bg-surface border border-border rounded-lg p-4 min-w-[160px] text-center hover:border-accent transition-colors cursor-pointer group"
                      >
                        <div className="text-xs font-[family-name:var(--font-mono)] text-muted mb-1 uppercase tracking-wide">
                          {node.type}
                        </div>
                        <div className="text-sm font-semibold text-text font-[family-name:var(--font-display)] group-hover:text-accent transition-colors">
                          {node.label}
                        </div>
                        {node.description && (
                        ...
                        {node.connections.length > 0 && (
                          <div className="mt-2 flex flex-wrap gap-1 justify-center">
                            {node.connections.map((conn) => (
                              <span
                                key={conn}
                                className="text-[10px] font-[family-name:var(--font-mono)] text-accent bg-accent/10 px-1.5 py-0.5 rounded"
                              >
                                → {conn}
                              </span>
                            ))}
                  ...
                  {testimonials.map((testimonial, i) => (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          <div className="text-sm font-semibold text-text font-[family-name:var(--font-display)]">
                            {testimonial.author}
                          </div>
                          <div className="text-xs text-muted font-[family-name:var(--font-mono)]">
                            {testimonial.role}
                          </div>
                        </div>
                          ...
                          <div className="flex gap-0.5">
                            {Array.from({ length: 5 }).map((_, j) => (
                              <span
                                key={j}
                                className={`text-sm ${
                                  j < testimonial.rating! ? 'text-accent' : 'text-muted/30'
                                }`}
                              >
                                ★
                              </span>
